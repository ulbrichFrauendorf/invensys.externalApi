...
namespace Invensys.ExternalApi.UnitTests.PaySpace
{
    public class PaySpaceApiAuthenticationServiceTests
    {
        ...
        public void SetUp()
        {
            ...
            _service = new PaySpaceApiAuthenticationService(_payspaceAuthenticationProviderMock.Object, _payspaceLookupApiMock.Object);
        }
        ...
        public async Task Authenticate_ShouldReturnAuthorizationResponse_WhenRequestIsValid()
        {
            ...
            _payspaceAuthenticationProviderMock.Setup(x => x.GetAuthenticationResult(accessTokenRequest, false))
                .ReturnsAsync(authenticationResult);

            _payspaceAuthenticationProviderMock.Setup(x => x.GetAccessToken(accessTokenRequest, false))
                .ReturnsAsync("accessToken");

            _payspaceLookupApiMock.Setup(x => x.GetOrganizationLevelsAsync(accessTokenRequest, 1))
