...
namespace Invensys.ExternalApi.UnitTests.PaySpace
{
   ...
   public class PayspaceApiClientTests
   {
      ...
      public void SetUp()
      {
         ...
         _payspaceApiClient = new PaySpaceApiClient(
      ...
      {
         var url = "https://api.payspace.com/data";
         var expectedData = new List<Data>
         {
            new() { Id = 1, Name = "Test" }
         };
         ...
         _httpMessageHandlerMock
               ...
               new HttpResponseMessage
               {
                  ...
                  Content = new StringContent(
                     JsonSerializer.Serialize(
                        ...
                        {
                           Value = new List<Data> { expectedData[0] },
                           OdataNextLink = new Uri("https://api.payspace.com/data?page=2")
                        }
                     )
                  )
               }
            ...
            .ReturnsAsync(
               new HttpResponseMessage
               {
                  StatusCode = HttpStatusCode.OK,
                  Content = new StringContent(
                     JsonSerializer.Serialize(new { value = new List<Data> { expectedData[1] } })
                  )
               }
            );
