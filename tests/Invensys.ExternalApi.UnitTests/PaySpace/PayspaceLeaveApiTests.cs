...
namespace Invensys.ExternalApi.UnitTests.PaySpace
{
   ...
   public class PayspaceLeaveApiTests
   {
      ...
      public async Task EmployeeLeaveApplicationListAsync_WithYearAndMonth_ShouldReturnLeaveApplications()
      {
         ...
         var year = 2023;
         var month = 1;
         var expectedApplications = new List<EmployeeLeaveApplication>
         {
            new EmployeeLeaveApplication { LeaveAdjustmentId = 1 }
         };

         _paySpaceApiClientMock
            .Setup(client =>
               client.GetListAsync<EmployeeLeaveApplication>(
                  _accessTokenRequest,
         ...
         var result = await _paySpaceLeaveApi.EmployeeLeaveApplicationListAsync(
            _accessTokenRequest,
            companyId,
            year,
            month
