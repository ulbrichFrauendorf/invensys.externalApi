...
public class Sage300ApiClient(
   IHttpClientFactory httpClientFactory,
   ISage300AuthenticationProvider sage300AuthenticationProvider
)
   : ExternalApiClient(
      httpClientFactory,
      sage300AuthenticationProvider,
      Sage300HttpClient.Sage300RateLimitedApi,
      HeaderType.Cookie,
      ...
      ISage300ApiClient
{
   ...
   public async Task<List<T>> GetListAsync<T>(
      ResourceOwnerPasswordCredentialTokenRequest accessTokenRequest,
      string url
   )
   {
      var response = await SendRequestWithAuthRetry<Sage300Response<T>>(
         accessTokenRequest,
         async () => await _httpClient.PostAsync(url, null)
      );
