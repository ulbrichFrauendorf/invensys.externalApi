...
public class PaySpaceApiClient(IHttpClientFactory httpClientFactory, IPaySpaceAuthenticationProvider paySpaceAuthenticationProvider)
   : ExternalApiClient(httpClientFactory, paySpaceAuthenticationProvider, PaySpaceHttpClient.PaySpaceRateLimitedApi, HeaderType.Authorization, TokenAppendType.Header)
   , IPaySpaceApiClient
{
 ...
 )
    {
        ...
        do
        {
            ...
            var filter = $"$filter={filterKey} in ('{string.Join("', '", partialFilters)}')";
    ...
    private async Task<List<T>> GetAllPagesWithAuthRetry<T>(JwtAccessTokenRequest accessTokenRequest, string initialUrl)
    {
        ...
        while (nextUrl != null)
        {
            var serializedResponse = await SendRequestWithAuthRetry<ListResponse<T>>(accessTokenRequest, async () => await _httpClient.GetAsync(nextUrl));
